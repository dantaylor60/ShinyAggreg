...
serverAgg <- function(input,output){
  ...
  allDomains <- reactive({
       ...
       if(input$group=="mosquito"){
         ...
         if(input$group=="pathogen"){
           ...
           if(input$typeFiles=="none"){
             ...
             if(otherFile!='400m2Data'){
               otherFile2='400m2Data'
             }
           }
    ...
    if(input$group=='plantPresenceCover' & otherFile!='400m2Data'){
      currentFiles2   <- filenames[grep(otherFile2,filenames)] 
      fileData2 <- matrix( unlist(strsplit(currentFiles2,'[.]')),length(currentFiles2),byrow=T)
    }
    ...
    dataAllDomains <- dataAllDomains2 <- dataAllField <- list()
    ...
    for(k in 1:nsite){
      ...
      if(input$group=='plantPresenceCover' & otherFile!='400m2Data'){
        kfile2  <- paste(input$group,'/',currentFiles2[k],sep='')
        kdat2 <- read.csv(kfile2,header=T,na.strings=c(""," "))
        dataAllDomains2[[site[k]]] <- kdat2
      }
    ...
    if(input$group=='plantPresenceCover' & otherFile!='400m2Data'){
      colDom2 <- ncol(dataAllDomains2[[1]])
      datares <- list()
      namvars <- colnames(dataAllDomains2[[1]])
      
      for(jj in 1:colDom2){
        datares[[jj]] <- unlist(lapply(dataAllDomains2,"[",,jj))
      }
      names(datares) <- namvars
      dataAllDomains2 <- data.frame(datares,stringsAsFactors = F)
    }
    ...
    if(otherFile%in%c('capturedata','IDandpinning')){
      ...
      if(!is.null(dataAllDomains2)){
        dataAllDomains2 = dataAllDomains2[!is.na(dataAllDomains2$eventID),]
      }
    }
    ...
    if(!is.null(dataAllDomains2)){
      dateValuesDom2 = chron(as.character(dataAllDomains2[,dateVar]),format=formatTime)
      dataAllDomains2$yr <- as.character(years(dateValuesDom2))
      dataAllDomains2$month <- as.numeric(months(dateValuesDom2))
    }
    if(input$group=="smallMammal"){
